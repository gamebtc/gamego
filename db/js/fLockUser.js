//lockUser(agentId int32, userId int32, obj udid)
let f = function (agentId, userId, udid) {
    const now = new Date();
    ...
    try {
        ...
        if (v1.acknowledged === true) {
            //插入登录日志
            db.recordLogin.insertOne(
                { _id: recordId, userId, inTime: now, outTime: now, udid: udid }
            );
        } else {
            //更新相同的键
