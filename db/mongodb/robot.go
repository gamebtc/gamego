...
func (d *driver) LoadRobot(room int32) *model.User {
	up := bson.D{
...
// 加载机器人
func (d *driver) LoadRobots(room int32, count int) []*model.User {
	up := bson.D{
		{"$set", bson.D{{"room", room}}},
		{"$inc", bson.D{{"ver", 1}}},
		lastTime,
...
// 卸载机器人
func (d *driver) UnloadRobot(room int32, id model.UserId) {
	query := bson.D{
		{"_id", id},
		{"room", room},
	}
...
// 卸载机器人
func (d *driver) UnloadRobots(room int32, ids []model.UserId) {
	query := bson.D{
		{"_id", bson.D{{"$in", ids}}},
		{"room", room},
	}
